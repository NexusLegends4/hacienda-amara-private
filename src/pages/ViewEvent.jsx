...
const ViewEvent = () => {
	...
	const downloadEventQr = () => {
		...
		const fileName = `${(event?.title || "event").replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").toLowerCase() || "event"}-qr.png`;
				...
				{showShareQr && event && (
					<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Share event">
						<div className="w-full max-w-sm rounded-3xl bg-white p-6 shadow-2xl">
							...
							<QRCodeCanvas ref={qrCanvasRef} value={eventUrl} size={1024} level="H" includeMargin className="hidden" />
							...
							<div className="mt-5 grid gap-3 sm:grid-cols-2">
								<button className="btn btn-outline rounded-full" onClick={downloadEventQr}><FiDownload /> Download PNG</button>
								<button className="btn btn-black rounded-full" onClick={shareEvent}><FiShare2 /> Share link</button>
							</div>
						</div>
					</div>
				)}

				...
				<div className="mx-auto max-w-7xl space-y-6">
					...
					<div className="rounded-[1.5rem] border border-black/5 bg-white/70 p-6 shadow-2xl backdrop-blur sm:rounded-[2rem] md:p-8">
						<div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
							<div className="max-w-2xl">
								<p className="text-sm font-medium uppercase tracking-[0.18em] text-base-content/55">Guest Experience</p>
								<h1 className="mt-3 text-3xl font-black tracking-tight text-base-content md:text-4xl">Resort Reviews</h1>
								...
								<div className="flex flex-col justify-center space-y-2">
									{[5, 4, 3, 2, 1].map((star, i) => (
										<div key={star} className="flex items-center gap-4">
											...
											<span className="w-8 text-right text-xs font-medium text-slate-400">{ratingSummary.counts[i]}</span>
										</div>
									))}
								</div>
							</div>
					...
					{loading ? (
						...
						<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
							{reviews.map((rev) => (
								<div key={rev.id} className="relative rounded-[2rem] border border-black/5 bg-white/80 p-6 shadow-xl backdrop-blur transition-all hover:shadow-2xl sm:p-8 flex flex-col justify-between overflow-hidden">
